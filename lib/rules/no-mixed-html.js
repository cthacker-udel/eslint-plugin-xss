    ...
    create(context) {
        ...
        var checkForXss = function (node, target) {
            ...
            nodes.forEach(function (childNode) {
                ...
                var msg =
                    "Unencoded input '{{ identifier }}' used in HTML context";
        ...
        var checkHtmlVariable = function (node) {
            var msg =
                "Non-HTML variable '{{ identifier }}' is used to store raw HTML";
            if (!isXssSafe(node)) {
                context.report({
                    node: node,
                    message: msg,
        ...
        var checkHtmlFunction = function (node, fault) {
            var msg =
                "Non-HTML function '{{ identifier }}' returns HTML content";
            if (!isXssSafe(node)) {
                context.report({
                    node: fault,
                    message: msg,
        ...
        var getXssCandidateParent = function (node) {
            ...
            for (var ptr = exprStack.length - 1; ptr >= 0; ptr--) {
                ...
                if (
                    functionRules.passthrough.obj &&
                    functionRules.passthrough.args
                )
                    continue;
